...
- `hooks/pre-start` creates `data/admin/config/node.env`, `keystore`, `identity`, and `logs`, reads any existing `node.env` before deriving the chain, generates `data/chain/chains/<chain>/network/secret_ed25519` on first start if missing, then chowns `data/` to `1000:1000`.
